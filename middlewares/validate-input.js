...
const validateInput = (req,res,next) => {
    ...
    {
        if(!isFinite(quanta))
        throw new InvalidInput(StatusCodes.BAD_REQUEST, "Provide valid quanta value!")
        
        quanta = Math.floor(Number(quanta))

        if(quanta<=0)
        throw new InvalidInput(StatusCodes.BAD_REQUEST, "Quanta can't be 0 or less!")
    }
